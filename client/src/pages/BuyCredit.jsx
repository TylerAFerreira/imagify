...
const BuyCredit = () => {
  ...
  return (
    <motion.div className='min-h-[80vh] text-center pt-14 mb-10'
      ...
      viewport={{ once: true }}
    >
      <button className='border border-gray-400 px-10 py-2 rounded-full mb-6'>Nos Abonnements</button>
      <h1 className='text-center text-3xl font-medium mb-6 sm:mb-10'>Choisir l'abonnement </h1>
      <div className='flex flex-wrap justify-center gap-6 text-left'>
        {plans.map((item, index) => (
          <div className='bg-white drop-shadow-sm border rounded-lg py-12 px-8 text-gray-600 hover:scale-105 transition-all duration-500' key={index}>
            ...
            <p className='mt-6'>
              <span className='text-3xl font-medium'>${item.price}</span>/ {item.credits} crédits
            </p>
